...
public class MyTree {
	...
	public void init() {
		...
		DefaultMutableTreeNode node = new DefaultMutableTreeNode(groupNames[0]);
		for (int j = 0; j < friendInfos.get(0).length; j++) {
			String friendName = friendInfos.get(0)[j];// 好友用户名
			String ip = "111";// 好友IP地址
			String port = "9999";// 好友的端口号
			String headImage = "0";// 好友头像
			String state = "1";// 状态

			nodeImages.add(friendName + "#" + "/client/img/" + headImage + "_32.jpg");
	...
	public void addFriendToTree(String groupName, String friendName) {
		int groupIndex = treeModel.getIndexOfChild((DefaultMutableTreeNode) treeModel.getRoot(),
				new DefaultMutableTreeNode(groupName));
		DefaultMutableTreeNode childNode = new DefaultMutableTreeNode(friendName);
		if (groupIndex != -1) {
			DefaultMutableTreeNode parent = (DefaultMutableTreeNode) treeModel.getChild(treeModel.getRoot(),
					groupIndex);
			treeModel.insertNodeInto(childNode, parent, parent.getChildCount());
		} else
